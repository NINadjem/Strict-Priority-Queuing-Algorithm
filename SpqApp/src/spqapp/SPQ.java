...
public class SPQ {
    ...
    private static double current_time; // temp actuel== notre horloge
    ...
    private int inputListCurrentIndex; // un p ointeur qui indique la colone courante
    private boolean done = false; // indicateur d'etat d'avancement du travail
    public static Character lastChar = ' ';
    private static Comparator simpleComparator = new Comparator<Character>() { // trier en ordre alphabetique
        @Override
        public int compare(Character o1, Character o2) {
            return Character.compare(o1, o2);
        }
    };
    public static Comparator comparator = new Comparator<Character>() { // trier en ordre alphabetique
        @Override
        public int compare(Character o1, Character o2) {
            System.out.println(o1+" **********"+o2+ " *********"+lastChar +"****"+current_time);
            int index = InputElement.alphabets.indexOf(lastChar);
            if (index == -1) {
                System.out.println(".compare()==-1");
                return Character.compare(o1, o2);
            } else {
                System.out.println(".compare()!=-1");
                int n=(Character.compare(lastChar, o1));
                int number=n*(Character.compare(lastChar,o2));
                if(number<0){
                    System.out.println(".compare() diffrent");
    ...
    private void getPassTheFirstWaitingElement(int elementIndexInInput) {
        ...
        inputElement.removeElementFromQueue(token_element); // suprimer l'element de la liste (liste d'attente de la colonne)
    ...
    private void getPassAllInWaitingList() {
        System.out.println("spqapp.SPQ.getPassAllInWaitingList()"+current_time+"***"+lastChar);//debug
        waiting_list.sort(comparator);
    ...
    public static class InputElement {
        ...
        public static ArrayList<Character> alphabets = new ArrayList<>();
        private double time; // temps
        ...
        public InputElement(double time, ArrayList<Character> waiting_list) {
            ...
            for (Character c : this.waiting_list) {
                if (!alphabets.contains(c)) {
                    alphabets.add(c);
                    System.err.println(c);
                }
            }
            alphabets.sort(simpleComparator);
        ...
        public void removeElementFromQueue(Character c) {
            if (!waiting_list.isEmpty()&&waiting_list.contains(c)) {
